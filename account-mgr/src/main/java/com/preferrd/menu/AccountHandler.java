...
@Component public class AccountHandler {
    ...
    public Mono<ServerResponse> getUsers(ServerRequest request) {
        ...
        return ok().contentType(MediaType.APPLICATION_JSON)
            .body(BodyInserters.fromObject(null == account ? "data not exist" : account));
    }

    public Mono<ServerResponse> putUsers(ServerRequest request) {
        Mono<Account> accountMono = request.bodyToMono(Account.class);
        accountMono.doOnSuccess(account -> accountService.addUser(account));
        //FIXME:deal error case.
        accountMono.doOnError(Throwable::printStackTrace);
