...
## 面试说辞
	...
	自我介绍
	我呢，16年本科毕业于杭州电子科技大学。15年开始实习，毕业后一直在目前这家公司供职做软件开发的工作。主要做的是Java偏后端。然后性格上呢周围朋友，同事，领导都觉得我是一个稳重踏实的人，做事比较理性，不过我自己觉得吧，还是理性加感性兼而有之，选择怎样的工作和生活还是要走心的，不然也很难做好一件事情，只是具体到事情本身，分析问题，处理的时候确实是更理性的思维方式。所以我做事风格上不是那种雷厉风行的风格，条件允许的情况下都是步步为营，先有初步的计划，然后才会具体落实。然后兴趣爱好上，比较喜欢历史，国学元素化的东西，运动呢喜欢打打羽毛球。平时会听听历史书，周末会和朋友打打羽毛球。另外的为人处世上就比较随和了，喜欢清风一样的人，自己也希望像清风一样。所以对人对事都还挺有包容性和耐心的。
	
	
	项目介绍
	最近做的一个项目是这个Platform，下面包含了common，security，logsys，dvm，fm，pm。我主要的工作负责整个Platform的架构和版本演进，包括根据需求调整架构设计，大平台升级，配合se完成接口设计，模型设计，完成一些功能模块，指导新人开发。这个项目本身是作为一个平台的化的东西，所以多数的实现是通用型的设计。主要想解决的是SDN网络系统中的一些通用场景，比如认证鉴权，日志管理，设备管理，服务注册发现，通知功能支持，持久化支持等等。项目呢是在ODL的平台上开发的，ODL架构本身基于OSGI，是SDN网络的一个实现平台。开发形式上这些独立的功能都是独立开发的，因此这些子项目都可以独立部署和运行，同样因为是基于OSGI，这些子项目也可以按照模块打包成bundle，这些bundle我们会发布在公司的maven仓库中。如果需要分布式的部署，那这些子项目build后单独部署就可以，如果是集成化的方式部署，一般也只需要将这些bundle作为依赖添加到需要集成的系统中就可以了，或者在构建feature时添加到集成的启动feature中。
